    ...
    Object.entries(antennasType).forEach(([k, antenna]) => {
        ...
        for (let i = 0; i < antenna.length; i++) {
            ...
            for (let j = 0; j < antenna.length; j++) {
                ...
                let ite = 0
                let deltaX = currentPoint.x + (currentPoint.x - comparePoint.x) * ite
                let deltaY = currentPoint.y + (currentPoint.y - comparePoint.y) * ite

                while (input[deltaX] != undefined && input[deltaX][deltaY] != undefined) {
                    ...
                    ite++;
                    deltaX = currentPoint.x + (currentPoint.x - comparePoint.x) * ite
                    deltaY = currentPoint.y + (currentPoint.y - comparePoint.y) * ite
                }
            }
        }
    });
