...
function isWorkingCalibration2(number, total, current) {
    ...
    let newTotalAdd = current + newValueAdd.shift()
    ...
    //Concatenation
    ////////////////
    let newValueConcat = number.slice()
    let newTotalConcat
    if (isStart) {
        newTotalConcat = newValueConcat.shift()
    } else {
        newTotalConcat = Number(String(current) + String(newValueConcat.shift()))
    }


    if (newTotalConcat == total && newValueConcat.length == 0) return true

    if (isWorkingCalibration2(newValueConcat, total, newTotalConcat)) {
        return true;
    };
}